...
def main():
    ...
    while run:
        ...
        if keys[pygame.K_a] and player.x - player_vel > 0: #izquierda
            player.x -= player_vel
        if keys[pygame.K_d] and player.x + player_vel + player.get_width() < HEIGHT: #derecha
            player.x += player_vel
        if keys[pygame.K_w] and player.y - player_vel > 0: #arriba
            player.y -= player_vel
        if keys[pygame.K_s] and player.y + player_vel + player.get_height() + 15 < HEIGHT: #abajo
            player.y += player_vel
...
def main_menu():
    ...
    while run:
        WIN.blit(BG, (0,0))
        title_label = title_font.render("Click al raton para iniciar...", 1, (255,255,255))
